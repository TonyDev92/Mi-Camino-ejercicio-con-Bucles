...
miCamino.material.forEach((e,i ) => {
  if(i == 0 && e.nombre == "Piolets"){
    console.log(`Lo primero que debo llevar es${e.nombre} y su descripción es ${e.descripcion}`)
  }else if( i == 1 && e.nombre =="Crampones"){
    console.log(`Lo segundo que debo llevar es ${e.nombre} y su descripción es ${e.descripcion}`)
  }else if( i == 2 && e.nombre == "Cuerdas"){
    console.log(`Lo tercero que debo llevar es ${e.nombre} y su descripción es ${e.descripcion}`)
  }else if (i == 3 && e.nombre == "Mochila"){
    console.log(`Lo último que debo llevar ${e.nombre} y su descripción ${e.descripcion}`)
  }
})
miCamino.cimas.forEach((e) => {
  if(e.nombre == "Everest"){
    console.log(`Para subir al ${e.nombre} debo subir ${e.altitud}metros y debo viajar a ${e.localizacion}`)
  }
})
miCamino.subiendoAlEverest.forEach((e) => {
  e.forEach((j)=>{
    j.campoBase.forEach((x) => {
      for(let i of x.campoI.campoII.campoIII.campoIV){
        console.log(i);
      }
    })
  })
...
miCamino.cimas.forEach((e , i) => {
    altitudTotal += e.altitud;
    totalElementos += i;
})
const mediaAltitud = ((a,b) => { let total = a / b; return total});
let resultado = mediaAltitud(altitudTotal,totalElementos);
